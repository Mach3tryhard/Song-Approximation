...
sr, data = wavfile.read('reconstructed_chords.wav')
scaling=5
k=3
#data = data[:100000]  # trim for performance
# data=data/20
...
reconstructed_left = spline_left(new_time).astype(datatype)
reconstructed=reconstructed_left
if data.ndim !=1:
    spline_right = make_interp_spline(new_time, new_right, k)
    reconstructed_right = spline_right(new_time).astype(datatype)
